...
const WeekendTips: React.FC = (): React.JSX.Element => {
    const [places, setPlaces] = useState<Places>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const navigation = useNavigation<NativeStackNavigationProp<HomeStackParamList>>();

    useEffect(() => {
        fetchWeekendPlaces().then();
    }, []);

    const fetchWeekendPlaces = async (): Promise<void> => {
        const { data, error } = await supabase
            .from("places")
            .select("id, title, description, image_url, city, country, price_usd, duration_days, distance_km")
            .lte("duration_days", 3)
            .order("price_usd", { ascending: true })
            .limit(12);

        if (error) {
            console.log("Error fetching weekend tips:", error.message);
        } else {
                    ...
                    >
                        <Card size="md" variant="elevated" className="rounded-3xl overflow-hidden">
                            <View className="w-full">
                                ...
                                {/* Gradient info */}
                                <LinearGradient colors={weekendTipsCardColor} className="p-4">
